class Api::V1::OrgAdminController < ApplicationController
  ...
  def index
    if search_request?
      ...
      @org_admins = OrgAdmin.search_range(params[:created_months_ago], :created_at)   if params[:created_months_ago]
      @org_admins = OrgAdmin.search_range(params[:updated_months_ago], :updated_at)  if params[:updated_months_ago]
      return @org_admins
    end
