@Learnster.module "Components.Loading", (Loading, App, Backbone, Marionette, $, _) ->
  ...
  class Loading.Controller extends App.Controllers.Base
    ...
    showRealView: (realView, loadingView, config) ->
      entities = if config.userWait and App.currentUser?._fetch?.status isnt 200 then [config.entities, App.currentUser] else config.entities

      App.execute "when:fetched", entities, =>
