class Activity < ActiveRecord::Base
  ...
  def self.course_manager_activities(admin_id)
    ...
    course_section_ids = CourseSection.where(course_id: course_ids).ids
    module_ids         = admin.learning_modules.ids
    supplement_ids     = ModuleSupplement.where(learning_module_id: module_ids).ids
    content_ids        = SupplementContent.where(module_supplement: supplement_ids).ids
